    ...
    with suppress(MessageIdInvalid, MessageNotModified, QueryIdInvalid):
        ...
        if query.data.startswith("info"):
            ...
            if nyav == 'main':
                ...
                await query.message.edit("<b>⚡️Showing help menu, Choose from the below options.</b>", reply_markup=reply_markup, disable_web_page_preview=True)
            elif nyav == 'play':
        ...
        if not query.from_user.id in admins:
            await query.answer(
                "Sorry! Bruh🥲, You Dont Have Permission",
                show_alert=True
                )
            return
        #scheduler stuffs
        if query.data.startswith("sch"):
            ...
            if data.startswith("sch_month"):
                ...
                if yea_r == "choose":
                    ...
                    await query.message.edit("⚡️Now Choose the month to schedule a voicechatㅤ ㅤㅤ", reply_markup=InlineKeyboardMarkup(button))
                elif day == "none":
                    return
                else:
                    year = int(yea_r)
                    ...
                    await query.message.edit(f"⚡️Choose the hour of {date} {smonth} {year} to schedule  a voicechat.", reply_markup=InlineKeyboardMarkup(button))
                ...
                button=[
                    [
                        InlineKeyboardButton("⚡️Confirm", callback_data=f"schconfirm_{year}-{month}-{day} {hour}:{minute}"),
                ...
                await query.message.edit(f"⚡️Succesfully scheduled to stream on <code> {date.strftime('%b %d %Y, %I:%M %p')} </code>")
                await delete_messages([query.message, query.message.reply_to_message])
                
            elif query.data == 'schcancelall':
                await cancel_all_schedules()
                await query.message.edit("⚡️All Scheduled Streams are cancelled succesfully.")

            elif query.data == "schcancel":
                buttons = [
                    [
                        InlineKeyboardButton('⚡️Yes, Iam Sure!!', callback_data='schcancelall'),
