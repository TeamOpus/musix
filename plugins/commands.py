...
async def start(client, message):
    if len(message.command) > 1:
        if message.command[1] == 'help':
            ...
            await message.reply("<b>⚡️Learn to use the VCPlayer Showing help menu, Choose from the below options⚙️</b>",
                                quote=False,
                                reply_markup=reply_markup,
                                disable_web_page_preview=True
                                )
        elif 'sch' in message.command[1]:
            msg = await message.reply("<b>⚡️Checking schedules.....</b>")
            _, me = message.command[1].split("_", 1)  # Splitting for extracting specific info
            who = Config.SCHEDULED_STREAM.get(me)
            if not who:
                return await msg.edit("<b>⚡️Something gone somewhere...</b>")
...
async def update_handler(client, message):
    if Config.HEROKU_APP:
        k = await message.reply("<b>⚡️Heroku APP found, Restarting app to update.</b>", quote=False)
        if Config.DATABASE_URI:
...
async def set_heroku_var(client, message):
    ...
    m = await message.reply("<b>⚡️Checking config vars....</b>")
    ...
    with suppress(MessageIdInvalid, MessageNotModified):
        ...
        if Config.DATABASE_URI and var in ["STARTUP_STREAM", "CHAT", "LOG_GROUP", "REPLY_MESSAGE", "DELAY", "RECORDING_DUMP", "QUALITY"]:      
            await m.edit("⚡️Mongo DB Found, Setting up config vars...")
            ...
            if not value:
                ...
                await m.edit(f"<b>⚡️Sucessfully deleted {var}</b>")
                await delete_messages([message, m])           
                return
            else:
                if var in ["CHAT", "LOG_GROUP", "RECORDING_DUMP", "QUALITY"]:
                    ...
                    if var == "QUALITY":
                        if Config.CALL_STATUS:
                            ...
                            if k == False:
                                await restart_playout()
                    await m.edit(f"⚡️Succesfully changed {var} to {value}")
                    await delete_messages([message, m])
                    return
                else:
            ...
            if not value:
                ...
                if var in config:
                    await m.edit(f"<b>⚡️Sucessfully deleted {var}</b>")
                    await asyncio.sleep(2)
                    await m.edit("<b>⚡️Now restarting the app to make changes.</b>")
                    if Config.DATABASE_URI:
                ...
                return
            if var in config:
                await m.edit(f"<b>⚡️Variable already found. Now edited to {value}</b>")
            else:
                await m.edit(f"<b>❌Variable not found, Now setting as new var.</b>")
            await asyncio.sleep(2)
            await m.edit(f"<b>⚡️Succesfully set {var} with value {value}, Now Restarting to take effect of changes...</b>")
            if Config.DATABASE_URI:
                msg = {"msg_id":m.message_id, "chat_id":m.chat.id}
                if not await db.is_saved("RESTART"):
                    db.add_config("RESTART", msg)
